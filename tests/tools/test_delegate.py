...
class TestChildCredentialPoolResolution(unittest.TestCase):
    ...
    def test_build_child_agent_excludes_inherited_mcp_when_opted_out(self, mock_cfg):
        parent = _make_mock_parent()
        ...
        self.assertEqual(MockAgent.call_args[1]["enabled_toolsets"], ["web", "browser"])

    @patch(
        "tools.delegate_tool._load_config",
        return_value={"inherit_mcp_toolsets": False},
    )
    def test_build_child_agent_keeps_explicit_mcp_when_opted_out(self, mock_cfg):
        parent = _make_mock_parent()
        ...
        self.assertEqual(
            MockAgent.call_args[1]["enabled_toolsets"], ["web", "mcp-MiniMax"]
        )
